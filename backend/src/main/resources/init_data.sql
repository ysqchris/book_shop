-- =============================================
-- Book Shop Sample Data - Yu Hua's Classic Books
-- Run this script after database tables are created
...
-- -----------------------------------------------
INSERT IGNORE INTO categories (id, name, parent_id, level, sort_order, icon, description, status, create_time, update_time, deleted)
VALUES
(1, '文学小说',  0, 1, 1, 'icon-literature', '文学类小说书籍',     0, NOW(), NOW(), 0),
(2, '科学技术',  0, 1, 2, 'icon-science',    '科学技术类书籍',     0, NOW(), NOW(), 0),
(3, '教材教辅',  0, 1, 3, 'icon-textbook',   '教材和教辅书籍',     0, NOW(), NOW(), 0),
(4, '生活艺术',  0, 1, 4, 'icon-art',        '生活艺术类书籍',     0, NOW(), NOW(), 0),
(5, '儿童读物',  0, 1, 5, 'icon-children',   '儿童读物书籍',       0, NOW(), NOW(), 0),
(6, '中国文学',  1, 2, 1, 'icon-cn-lit',     '中国文学小说',       0, NOW(), NOW(), 0),
(7, '外国文学',  1, 2, 2, 'icon-foreign-lit','外国文学小说',       0, NOW(), NOW(), 0),
(8, '计算机',    2, 2, 1, 'icon-computer',   '计算机技术书籍',     0, NOW(), NOW(), 0),
(9, '医学',      2, 2, 2, 'icon-medical',    '医学类书籍',         0, NOW(), NOW(), 0),
(10,'大学教材',  3, 2, 1, 'icon-college',    '大学教材书籍',       0, NOW(), NOW(), 0),
(11,'高中教材',  3, 2, 2, 'icon-highschool', '高中教材书籍',       0, NOW(), NOW(), 0);

-- -----------------------------------------------
-- Insert admin user (password: 123456, bcrypt)
-- -----------------------------------------------
INSERT IGNORE INTO users (id, username, password, email, phone, real_name, role, status, create_time, update_time, deleted)
VALUES
(1, 'admin', '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW',
 'admin@bookshop.com', '13800138000', '书店管理员', 1, 0, NOW(), NOW(), 0);

-- -----------------------------------------------
-- Insert Yu Hua's classic books + other samples
-- -----------------------------------------------
INSERT IGNORE INTO books (id, title, author, isbn, publisher, publish_date, category_id,
...
SELECT id, name, level FROM categories WHERE deleted = 0;
SELECT '=== Yu Hua Books ===' AS info;
SELECT id, title, author, price, stock FROM books WHERE author = '余华' AND deleted = 0;
